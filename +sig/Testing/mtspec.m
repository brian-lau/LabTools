...
%%%%%%%% Oversample frequencies within bandwidth, 79 frequencies
nxi = 79;
...
for k = 1:kspec
   xk(:,k) = wt(1:nf,k).*yk(1:nf,k); % wt=weights from adapt, yk = fourier(x.*tapers)
   Vk(:,k) = fftshift(fft(vn(:,k),nfft)); %vn=tapers, Vk=fourier(tapers)
end

%%%%%%%% Interp fourier transformed tapers to new grid
